...
public abstract class ApplicationFlow extends LoggingLayer implements ApplicationFlowInt {
	...
	protected Result parseAndSetElements(Payload request) {
		...
		if (!table.containsKey(Constants.CLIENT_ID)) {
			...
			result.setThrowable(
					new ServicesException("Missing Client-Secret in payload.", ErrorCodes.MISSING_PARAM, null));
			exiting(methodName, entryTime, result);
			return result;
		} else if (!table.containsKey(Constants.ENVIRONMENT)) {
			error("Missing Environment Type in payload.", result);
			result.setThrowable(
					new ServicesException("Missing Environment Type in payload.", ErrorCodes.MISSING_PARAM, null));
			exiting(methodName, entryTime, result);
			return result;
		}
		...
		try {
			...
			result.setThrowable(new ServicesException(
					"Unable to decrypt clientID and clientSecret. Make sure there is not a casting error to <String>",
					ErrorCodes.BASIC_ERROR, e));
			exiting(methodName, entryTime, result);
			return result;
		}
