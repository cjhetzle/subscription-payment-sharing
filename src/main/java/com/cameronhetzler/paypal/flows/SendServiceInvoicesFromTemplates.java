...
public class SendServiceInvoicesFromTemplates extends ApplicationServiceFlow {
	...
	private Result buildBaseInvoiceRequest(Payload request) {
		...
		try {
			...
			result.setThrowable(
					new ServicesException("Unable to cast service <Object> to <String>.", ErrorCodes.BASIC_ERROR, e));
			exiting(methodName, entryTime, result);
			return result;
		} finally {
			if (service == null) {
		...
		try {
			...
			error("Exception caught.", e);
			result.setThrowable(new ServicesException(
					"Unable to load MerchantInfo file for service: " + this.service + ". Make sure file exists.",
					ErrorCodes.FILE_READ, e));
			exiting(methodName, entryTime, result);
			return result;
		}
		...
		try {
			...
			error("Exception caught.", e);
			result.setThrowable(new ServicesException(
					"Unable to load Tax file for service: " + this.service + ". Make sure file exists.",
					ErrorCodes.FILE_READ, e));
			exiting(methodName, entryTime, result);
			return result;
		}
		...
		try {
			...
			error("Exception caught.", e);
			result.setThrowable(new ServicesException(
					"Unable to load Item file for service: " + this.service + ". Make sure file exists.",
					ErrorCodes.FILE_READ, e));
			exiting(methodName, entryTime, result);
			return result;
		}
		...
		try {
			...
			error("Exception caught.", e);
			result.setThrowable(new ServicesException(
					"Unable to load Invoice file for service: " + this.service + ". Make sure file exists.",
					ErrorCodes.FILE_READ, e));
			exiting(methodName, entryTime, result);
			return result;
		}
	...
	private Result getBillingInfoAndSend(Payload request) {
		...
		try {
			...
			result.setThrowable(new ServicesException(
					"Unable to load BillingInfo file for service: " + this.service + ". Make sure file exists.",
					ErrorCodes.FILE_READ, e));
			exiting(methodName, entryTime, result);
			return result;
		}
		...
		try {
			...
			result.setThrowable(new ServicesException(
					"Error while creating Invoices. Make sure template data is correct.", ErrorCodes.BASIC_ERROR, pre));
			exiting(methodName, entryTime, result);
			return result;
		}
